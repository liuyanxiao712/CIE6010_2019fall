...
A few obervation regarding $p(u)$:
...
\begin{proof}[proof of (3)]
	...
	Let $\tilde{x} = \lambda x_1 + (1-\lambda)x_2$, where $x_1\in \text{dom}\ F_1\cap \text{dom}\ F_0\cap \{x\vert F_1(x)\leq u_1 \}$, $x_2\in \text{dom}\ F_1\cap \text{dom}\ F_0\cap \{x\vert F_1(x)\leq u_2 \}$.
	
	So we can write
	\begin{align*}
		F_1(\tilde{x}) 
		&= F_1(\lambda x_1 + (1-\lambda)x_2) &(\text{Since } \tilde{x}\in \text{dom}\ F_1)\\
		&\leq \lambda F_1(x_1) + (1-\lambda)F_1(x_2) &(\text{Since } F_1 \text{ is convex})\\
		&\leq \lambda u_1 + (1-\lambda)u_2 &(\text{Since } F_1(x_i) \leq u_i, i=1, 2)
	\end{align*}
...
Think about trade-off between $F_1(x)$ and $F_0(x)$ in a slightly different way:
\begin{align*}
